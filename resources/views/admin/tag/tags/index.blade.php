                ...
                <div class="ibox ">
                    <div class="ibox-title">
                        ...
                        <div class="ibox-tools">
                            @if(App\Helpers\Checker::checkAcces('tags','create')) 
                                <a class="" href="{{ url(Config::get('constants.ADMIN_PATH').'tags'.'/create') }}">
                    ...
                    <div class="ibox-content">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover dataTables-example" >
                                <thead>
                                    <tr>
                                        <th>Libelle</th><th><i class="fa fa-wrench"></i></th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <td>{{ $item->libelle }}</td>
                                         ...
                                         <div class="btn-group">
                                                ...
                                                @if(App\Helpers\Checker::checkAcces('tags','edit')) 
                                                    <li>
                                                        <a class="dropdown-item" href="{{ url(Config::get('constants.ADMIN_PATH').'tags/' . $item->id . '/edit') }}" title="Edit Tag"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editer</a>
                                                    </li>
                                                @endif
                                                ...
                                                @if(App\Helpers\Checker::checkAcces('tags','destroy')) 
                                                    ...
                                                    <li>
                                                        <form method="POST" action="{{ url(Config::get('constants.ADMIN_PATH').'tags' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                            {{ method_field('DELETE') }}
                                                            {{ csrf_field() }}
...
@section('script')
    ...
    <script>
        $(document).ready(function(){
            $('.dataTables-example').DataTable({
                ...
                buttons: [
                @if(App\Helpers\Checker::checkAcces('tags','files'))
                    { extend: 'copy'},
                    {extend: 'csv'},
    ...
    @if(App\Helpers\Checker::checkAcces('tags','show')) 
        ...
        <script>
            ...
            $("tr td:not(:nth-last-child(1))").on("click", function() {
              if (((new Date().getTime()) - touchtime) < 300) {
                    ...
                    $.ajax({
                                url: "{{ url(Config::get('constants.ADMIN_PATH').'tags/') }}"+'/'+_key,
                            })
                    .done(function(data) {
